...
-   Fix issue where loading a new image would not recompute the scrambled image used for image redaction.
...
-   Fix issue where zooming would still be animated even with `animations` prop set to `"never"`.

## 8.94.3 | 2025-10-06

-   Fix issue where `CSSFontFaceRule` with missing style prop would throw an error.

## 8.94.2 | 2025-10-03

-   Fix issue where Pintura would throw an error if stylesheets contained invalid `@font-face` rules (missing `src` or `font-family` props).
...
-   Fix issue with TypeScript throwing an error when importing defaults from locale directories.
