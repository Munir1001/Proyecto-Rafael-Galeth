  ...
  return (
    ...
    >
      <div className="min-h-screen flex bg-slate-50 overflow-x-hidden font-sans">
        ...
        <div className="flex-1 flex items-center justify-center px-4 sm:px-6 bg-white overflow-hidden shadow-lg z-10">
          <div className="w-full max-w-sm sm:max-w-md flex flex-col justify-center min-h-screen sm:min-h-0 sm:h-full sm:max-h-screen py-4 sm:py-6">
            ...
            <div className="mb-6 sm:mb-8 text-center shrink-0">
              <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-gray-900 mb-2">Bienvenido</h2>
              <p className="text-gray-500 text-xs sm:text-sm">Accede a tu panel de trabajo.</p>
            </div>
            ...
            {errorMsg && (
              <div className="mb-4 p-3 bg-red-50 border-l-4 border-red-500 rounded-r-lg flex items-start gap-3 shrink-0 animate-pulse-once">
                ...
                <span className="text-red-700 text-xs sm:text-sm font-medium">{errorMsg}</span>
              </div>
            )}
            ...
            <div className="space-y-5">
              ...
              <button
                onClick={handleLogin}
                disabled={loading}
                className="w-full flex items-center justify-center px-4 sm:px-6 py-3 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white font-bold rounded-xl shadow-md transition-all duration-200 transform hover:scale-105 disabled:opacity-70 disabled:hover:scale-100 disabled:cursor-not-allowed mt-4 mobile-button"
              >
                {loading ? (
                  <div className="h-5 w-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                ) : (
                  <>
                    <LogIn className="mr-2 h-4 w-4 sm:h-5 sm:w-5" /> <span className="text-sm sm:text-base">Iniciar Sesión</span>
                  </>
                )}
              </button>
            </div>

            <div className="text-center mt-6 sm:mt-8 p-3 bg-slate-100 rounded-lg shrink-0">
              <p className="text-xs text-slate-600 mb-1">
                ¿No posees una cuenta institucional?
              </p>
              <Link
                to="/register"
                className="inline-flex items-center gap-2 font-semibold text-indigo-700 hover:text-indigo-800 transition-colors hover:underline text-sm mobile-button"
              >
